...
## 実行結果
実験を行い得られた例を./resultファイル内に保存しました。
例として保存した２つのフォルダでは、報酬をθとしたものと報酬をθ*fsとしたものです。
このパラメータの変更の仕方は、README.mdの21~25行目のように変更可能です。
２つの実験では、stepreward.pngで1エピソードを1440ステップとしたときの累計報酬をプロットしました。
